...
// Section Heading
function br_section_heading( $title = '', $subtitle = '' ) {
	if( $title || $subtitle ) :
	?>
		<div class="container">
			<div class="row">
...
// Enqueue scripts
add_action( 'wp_enqueue_scripts', 'br_companion_frontend_scripts', 99 );
function br_companion_frontend_scripts() {

	wp_enqueue_script( 'br-companion-script', plugins_url( '../js/loadmore-ajax.js', __FILE__ ), array( 'jquery' ), '1.0', true );
	wp_enqueue_script( 'br-common-js', plugins_url( '../js/common.js', __FILE__ ), array( 'jquery' ), '1.0', true );

}
// 
add_action( 'wp_ajax_br_portfolio_ajax', 'br_portfolio_ajax' );

add_action( 'wp_ajax_nopriv_br_portfolio_ajax', 'br_portfolio_ajax' );
function br_portfolio_ajax( ){
	...
	function br_update_the_followed_post_page_status( $title = 'Hello world!', $type = 'post', $status = 'draft', $message = false ){
...
function listing_custom_posts() {	
	$labels = array(
		'name'               => _x( 'Listing', 'post type general name', 'br-companion' ),
		'singular_name'      => _x( 'Listing', 'post type singular name', 'br-companion' ),
		'menu_name'          => _x( 'Listing', 'admin menu', 'br-companion' ),
		'name_admin_bar'     => _x( 'Listing', 'add new on admin bar', 'br-companion' ),
		'add_new'            => _x( 'Add New', 'br', 'br-companion' ),
		'add_new_item'       => __( 'Add New Listing', 'br-companion' ),
		'new_item'           => __( 'New Listing', 'br-companion' ),
		'edit_item'          => __( 'Edit Listing', 'br-companion' ),
		'view_item'          => __( 'View Listing', 'br-companion' ),
		'all_items'          => __( 'All Listings', 'br-companion' ),
		'search_items'       => __( 'Search Listing', 'br-companion' ),
		'parent_item_colon'  => __( 'Parent Listing:', 'br-companion' ),
		'not_found'          => __( 'No Listing found.', 'br-companion' ),
		'not_found_in_trash' => __( 'No Listing found in Trash.', 'br-companion' )
	);

	$args = array(
		'labels'             => $labels,
		'description'        => __( 'Description.', 'br-companion' ),
		'public'             => true,
	...
	$labels = array(
		'name'              => _x( 'Listing Category', 'taxonomy general name', 'br-companion' ),
		'singular_name'     => _x( 'Listing Categories', 'taxonomy singular name', 'br-companion' ),
		'search_items'      => __( 'Search Listing Categories', 'br-companion' ),
		'all_items'         => __( 'All Listings Categories', 'br-companion' ),
		'parent_item'       => __( 'Parent Listing Category', 'br-companion' ),
		'parent_item_colon' => __( 'Parent Listing Category:', 'br-companion' ),
		'edit_item'         => __( 'Edit Listing Category', 'br-companion' ),
		'update_item'       => __( 'Update Listing Category', 'br-companion' ),
		'add_new_item'      => __( 'Add New Listing Category', 'br-companion' ),
		'new_item_name'     => __( 'New Listing Category Name', 'br-companion' ),
		'menu_name'         => __( 'Listing Category', 'br-companion' ),
	);
	...
	$labels = array(
		'name'              => _x( 'Listing Country', 'taxonomy general name', 'br-companion' ),
		'singular_name'     => _x( 'Listing Countries', 'taxonomy singular name', 'br-companion' ),
		'search_items'      => __( 'Search Listing Countries', 'br-companion' ),
		'all_items'         => __( 'All Listings Countries', 'br-companion' ),
		'parent_item'       => __( 'Parent Listing Country', 'br-companion' ),
		'parent_item_colon' => __( 'Parent Listing Country:', 'br-companion' ),
		'edit_item'         => __( 'Edit Listing Country', 'br-companion' ),
		'update_item'       => __( 'Update Listing Country', 'br-companion' ),
		'add_new_item'      => __( 'Add New Listing Country', 'br-companion' ),
		'new_item_name'     => __( 'New Listing Country Name', 'br-companion' ),
		'menu_name'         => __( 'Listing Country', 'br-companion' ),
	);
...
/*=========================================================
    Cases Section
========================================================*/
function br_case_section( $post_order ){ 
	...
	if( $cases->have_posts() ) {
		while ( $cases->have_posts() ) {
			...
			$case_img      = get_the_post_thumbnail( get_the_ID(), 'br_case_study_thumb_362x240', '', array( 'alt' => get_the_title() ) );
			?>
...
// Related br for Single Page
function br_related_items( $current_post_id = null, $post_item = 3, $post_order = 'DESC', $have_related_listing_title = 'Related Listings' ){
			...
			<div class="row">
				...
				if( $related_listings->have_posts() ) {
					while ( $related_listings->have_posts() ) {
						...
						$recipe_img = get_the_post_thumbnail( get_the_ID(), 'br_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
						$listing_address = ! empty( br_meta( 'listing_address') ) ? br_meta( 'listing_address') : 'N/A';
						$phone_number = ! empty( br_meta( 'phone_number') ) ? br_meta( 'phone_number') : 'N/A';
						$listing_email = ! empty( br_meta( 'listing_email') ) ? br_meta( 'listing_email') : 'N/A';
						?>
...
function br_get_tabbed_contents( $sec_title = 'Explore Europe', $selected_countries ) {
	$i = 0;
	?>
						...
						<div class="row">
							...
							if( $tabbed_listings->have_posts() ) {
								while ( $tabbed_listings->have_posts() ) {
									...
									$listing_img = get_the_post_thumbnail( get_the_ID(), 'br_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
									$listing_address = ! empty( br_meta( 'listing_address') ) ? br_meta( 'listing_address') : 'N/A';
									$phone_number = ! empty( br_meta( 'phone_number') ) ? br_meta( 'phone_number') : 'N/A';
									$listing_email = ! empty( br_meta( 'listing_email') ) ? br_meta( 'listing_email') : 'N/A';
									?>
	...
	function search_prop_form_datas() {
		...
		$properties = new WP_Query( array(
			...
			'meta_query' => array(
				'relation' => 'OR',
				array(
					'key'     => '_br_listing_price',
					'value'   => array(1000, 1000000),
					'type'    => 'numeric',
					'compare' => 'BETWEEN',
				),
				array(
					'key'     => '_br_listing_area',
					'value'   => array(1000, 1000000),
					'type'    => 'numeric',
					'compare' => 'BETWEEN',
				),
		...
		if( $properties->have_posts() ) {
			while ( $properties->have_posts() ) {
				$properties->the_post();		
				$property_img = get_the_post_thumbnail( get_the_ID(), 'br_listing_thumb_362x250', '', array( 'alt' => get_the_title() ) );
